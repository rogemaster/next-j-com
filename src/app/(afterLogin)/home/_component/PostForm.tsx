...
export default function PostForm({ me }: Props) {
  ...
  const mutation = useMutation({
    ...
    async onSuccess(response, variable) {
      ...
      if (queryClient.getQueryData(['posts', 'recommended'])) {
        queryClient.setQueryData(['posts', 'recommended'], (prevData: { pages: IPost[][] }) => {
          const shallow = {
            ...prevData,
            ...
            (v, index) =>
              v && (
                <div key={index} style={{ flex: 1 }} onClick={() => onRemoveImage(index)}>
                  <img style={{ width: '100%', objectFit: 'contain', maxHeight: 100 }} src={v.dataUrl} alt="미리보기" />
                </div>
              ),
          )}
        </div>
